...
const createTransaction = TryCAtch(async (req, res) => {
    ...
    const { reason, out_time, in_time, login } = req.body;

    // console.log(mongoose.Types.ObjectId.isValid(login), "  --> ", login)
    if (!(mongoose.Types.ObjectId.isValid(login) && (/^[0-9a-fA-F]{24}$/.test(login)))) {
        // console.log(login)
        res.status(Status.VALIDATION_ERROR)
        throw new Error("Invalid login id")
    }
    const count = await TransactionModel.find({ login: new mongoose.Types.ObjectId(login) }).countDailyPass()
    ...
    const transaction = new TransactionModel({ reason, out_time, in_time, login })
...
const getOneStudAllTran = TryCAtch(async (req, res) => {
    ...
    result = result.map((doc) => {
        ...
        doc.actual_in_time = doc.actual_in_time? new Date(doc.actual_in_time).toLocaleString(): undefined
        doc.actual_out_time = doc.actual_out_time? new Date(doc.actual_out_time).toLocaleString(): undefined
        return doc
    })
...
const updateStatus = TryCAtch(async (req, res) => {
    ...
    if(!["Accepted", "Rejected"].includes(status)) {
        ...
        throw new Error("warden allowed to update status either Allowed or Rejected")
    }
    if (!(mongoose.Types.ObjectId.isValid(t_id) && (/^[0-9a-fA-F]{24}$/.test(t_id)))) {
        // console.log(t_id)
        res.status(Status.VALIDATION_ERROR)
    ...
    if (!["Accepted", "Rejected"].includes(status)) {
        res.status(Status.UNAUTHORIZED)
        throw new Error("Only Accepted and Rejected are allowed")
    }

    const result = await TransactionModel.updateOne({ _id: t_id, status: {$in: ["Pending"]} }, { $set: { status } })
...
const generateQr = TryCAtch(async (req, res) => {
    ...
    let result = await TransactionModel.findOne({ _id: t_id, status: { $in: ["Accepted", "Check_Out"] } })
    console.log(result)
    if (!result) {
        res.status(Status.NOT_FOUND)
    ...
    console.log((result.status == "Accepted"), " ", new Date(result.createdAt).toLocaleString(), " ", new Date(startOfDay).toLocaleString(), " ", new Date(endOfDay).toLocaleString(), " ", !((startOfDay <= result.createdAt) || (result.createdAt <= endOfDay)))
    if ((result.status == "Accepted") && ((startOfDay <= result.createdAt) && (result.createdAt <= endOfDay))) {
        ...
        await TransactionModel.updateOne({ _id: t_id }, { $set: { status: "Expired" } });
        res.status(Status.UNAUTHORIZED)
...
const verifyQr = TryCAtch(async (req, res) => {
    ...
    try {
        ...
        if (result.status == "Accepted" && payload.status == "Accepted") {
            ...
            await TransactionModel.updateOne( { _id: payload._id }, { $set: { status: updated_status, actual_out_time: new Date() } } )

        }
        else if (result.status == "Check_Out" && payload.status == "Check_Out") {

            updated_status = "Check_In"
            await TransactionModel.updateOne({ _id: payload._id }, { $set: { status: updated_status, actual_in_time: new Date() } })

        } else {

            res.status(Status.UNAUTHORIZED)
            throw new Error(`Last Qr action is - ${result.status} please generate new Qr for further action`)
            
        }
        const student_info = await StudentModel.findOne({ login: result.login }).populate("login")

        return res.Response(Status.SUCCESS, `Qr Code Verified and Status updated to - ${updated_status}`, student_info)
...
module.exports = { createTransaction, getALL, getOneStudAllTran, generateQr, updateStatus, verifyQr }